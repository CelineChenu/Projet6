...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(isset($_SESSION['user']['id'])){
        ...
        if(isset($donnees->content) && !empty($donnees->content)){
            ...
            // On écrit la requête
            $chats = $this->getDoctrine()->getRepository(\App\Entity\Chat::class)->findAll();


            'INSERT INTO `contents`(`content`, `users_id`) VALUES (:content, :user);';

            // On prépare la requête
            $query = $db->prepare($sql);

            // On injecte les valeurs
            $query->bindValue(':content', strip_tags($donnees->content), PDO::PARAM_STR);
